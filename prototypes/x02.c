...
void get_hardware_port_name(const char *ifname, char *buf, size_t len) {
    ...
    io_service_t parent = IORegistryEntryGetParentEntry(service, kIOServicePlane);
    int found = 0;
    ...
    while (parent != MACH_PORT_NULL) {
        ...
        if (prop) CFRelease(prop); // Not a string, continue up

        io_service_t next_parent = IORegistryEntryGetParentEntry(parent, kIOServicePlane);
        IOObjectRelease(parent);
        parent = next_parent;
    }

    if (!found) {
        snprintf(buf, len, "%s", ifname); // Fallback to interface name
    }
...
int get_mac_address(const char *ifname, char *mac_str, size_t len) {
    ...
    ifr.ifr_addr = (struct sockaddr *)&ss;
    ...
    if (ioctl(fd, SIOCGIFLLADDR, &ifr) == 0) {
        ...
        if (sdl->sdl_family == AF_LINK && sdl->sdl_alen > 0) {
            ...
            const unsigned char *addr = LLADDR(sdl);
...
int main(void) {
    ...
    if (colors) {
        printf("%s%-14s %-28s %-18s %-18s %-8s%s\n",
               BOLD_CYAN, "Interface", "Hardware Port", "IPv4 Address", "MAC Address", "Status", RESET);
        printf("%s%-14s %-28s %-18s %-18s %-8s%s\n",
               BOLD_CYAN, "---------", "-------------", "----------", "---------", "------", RESET);
    } else {
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        if (!(ifa->ifa_flags & IFF_UP)) continue; // Only active interfaces
